...
def generator(z, batch_size, z_dim):
    ...
    # 101-dim vector to 12288-dim (64x64x3)
    g_w1 = tf.get_variable('g_w1', [z_dim+1, 12288], dtype=tf.float32, 
...
def discriminator(images, reuse_variables=None):
    with tf.variable_scope(tf.get_variable_scope(), reuse=reuse_variables) as scope:
        ...
        d_w4_1 = tf.get_variable('d_w4_1', [1024, 1], 
                                  initializer=tf.truncated_normal_initializer(stddev=0.02))
        d_b4_1 = tf.get_variable('d_b4_1', [1], 
                                  initializer=tf.constant_initializer(0))

        d_dis = tf.matmul(d3, d_w4_1)
        d_dis = tf.nn.bias_add(d_dis, d_b4_1)

        # auxiliary: class
        d_w4_2 = tf.get_variable('d_w4_2', [1024, 1], 
                                  initializer=tf.truncated_normal_initializer(stddev=0.02))
        d_b4_2 = tf.get_variable('d_b4_2', [1], 
                                  initializer=tf.constant_initializer(0))

        d_ac = tf.matmul(d3, d_w4_2)
        d_ac = tf.nn.bias_add(d_ac, d_b4_2)

        #print("d4 shape: ", d_dis.shape)
